...
describe 'certbot::hook::deploy' do
  ...
  on_supported_os.each do |os, facts|
    context "on #{os}" do
      ...
      context 'with certbot class included' do
        ...
        it { is_expected.to contain_certbot__hook('deploy test') }
        it { is_expected.to contain_file('/etc/letsencrypt/renewal-hooks/deploy/test') }
      end
    end
  end
end
